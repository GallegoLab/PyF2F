...
from skimage.segmentation import find_boundaries, expand_labels, mark_boundaries
import time
import os
...
def convert_to_image(rlefile, outputdirectory, preprocessed_image_list,
    ...
    for f in files:
        ...
        # Custom: retrieve contour and save image
        image_c = image.astype(np.uint8)
        contour = find_boundaries(image_c, mode='inner').astype(np.uint8)  # find contour as 1 in the surface
        imsave(outputdirectory + "contour_{}.tif".format(f),
               contour, plugin="tifffile", check_contrast=False)

        # Save mask image in directory masks/
