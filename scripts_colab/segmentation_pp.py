...
def main_segmentation(segment_dir, images_dir, spots_dir, results_dir, figures_dir, scale_factor,
    ...
    sel_distances_to_contour = list()
    non_sel_distances_to_contour = list()
    total_data = 0
    total_selected = 0
        ...
        for img_file in glob.glob(segment_dir + "masks/image_*"):
            ...
            if os.path.exists(f'{spots_dir}detected_spot_{image_number}_W1_warped.csv') and os.path.exists(
                ...
                selection_df_W2, sub_df_W2, percent_sel_W2 = sort_by_distances(spots_df_W2,
                                                                               ...
                                                                               cont_cutoff=cont_cutoff,
                                                                               neigh_cutoff=neigh_cutoff)
                # print(selection_df_W1, sub_df_W1)
                sel_distances_to_contour += sub_df_W1.loc[sub_df_W1["selected"] == "sel", "dist_cont"].tolist()
                non_sel_distances_to_contour += sub_df_W1.loc[sub_df_W1["selected"] == "non-sel", "dist_cont"].tolist()
                # print(sel_distances_to_contour)
    ...
    print("\n\nTotal Percent W1 --> {} %\n"
          ...
          "Total Paired Percent --> {} % ({} spots) \n".format(sum(percent_sel_total_W1) / len(percent_sel_total_W1),
                                                               ...
                                                               sum(percent_sel_total) / len(percent_sel_total),
                                                               total_selected))
    np.savetxt(f"{results_dir}sel_distances_to_contour.csv", sel_distances_to_contour, delimiter=",")
    np.savetxt(f"{results_dir}non-sel_distances_to_contour.csv", non_sel_distances_to_contour, delimiter=",")
